...
export class ConcurrencyParserService extends AbstractParser<ConcurrencyRelation> {
  ...
  protected processFileLines(
    lines: Array<string>
  ): ConcurrencyRelation | undefined {
    ...
    for (const line of lines) {
      ...
      const eventA = this.getUniqueLabel(
        match[1],
        parseInt(match[2]),
        relabeler
      );
      const eventB = this.getUniqueLabel(
        match[3],
        parseInt(match[4]),
        relabeler
      );
