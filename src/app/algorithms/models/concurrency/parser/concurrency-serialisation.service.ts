...
export class ConcurrencySerialisationService {
  ...
  public serialise(concurrency: ConcurrencyRelation): string {
    ...
    this.iterateConcurrentEntries(
      matrices.unique,
      true,
      (labelA, labelB, fab, fba) => {
        const originalA = this.getOriginalLabel(
          labelA,
          cachedUniqueLabels,
          relabeler
        );
        const originalB = this.getOriginalLabel(
          labelB,
          cachedUniqueLabels,
          relabeler
        );

        result += this.formatConcurrencyEntry(
          this.formatUniqueLabel(originalA),
          this.formatUniqueLabel(originalB),
          fab!,
          fba!
        );
      }
    ...
    this.iterateConcurrentEntries(
      matrices.mixed,
      false,
      (wildcardLabel, uniqueLabel) => {
        // TODO unmapping of wildcard labels might be needed
        const uniqueOriginal = this.getOriginalLabel(
          uniqueLabel,
          cachedUniqueLabels,
          relabeler
        );
        ...
        result += this.formatConcurrencyEntry(
          wildcardLabel,
          this.formatUniqueLabel(uniqueOriginal)
        );
      }
    );
  ...
  protected iterateConcurrentEntries(
    matrix: ConcurrencyMatrix,
    symmetric: boolean,
    consumer: (a: string, b: string, fab?: number, fba?: number) => void
  ) {
    if (!symmetric) {
  ...
  protected processMatrixEntry(
    ...
    if (typeof matrix[labelA][labelB] === 'boolean') {
      ...
      consumer(
        labelA,
        labelB,
        matrix[labelA][labelB] as number,
        matrix[labelB][labelA] as number
      );
    }
  }

  protected getOriginalLabel(
    label: string,
    cachedUniqueLabels: Map<string, OrderedOriginal>,
    relabeler: Relabeler
  ): OrderedOriginal {
    ...
    if (original === undefined) {
      ...
      console.debug(label);
      throw new Error(
        'Unique concurrency matrix contains an entry unknown to the relabeling function!'
      );
    }
    const order = relabeler
      .getLabelOrder()
      .get(original)!
      .findIndex((l) => l === label);
    if (order === -1) {
      ...
      console.debug(label);
      throw new Error(
        'Unique concurrency matrix contains an entry outside of the relabeling order of the relabeling function!'
      );
    }
  ...
  protected formatConcurrencyEntry(
    formattedLabelA: string,
    formattedLabelB: string
  ): string;
  protected formatConcurrencyEntry(
    formattedLabelA: string,
    formattedLabelB: string,
