...
import { PetriNetIsomorphismService } from '../petri-net/isomorphism/petri-net-isomorphism.service';
...
export class LogToPartialOrderTransformerService extends LogCleaner {
  ...
  public transformToPartialOrders(
    ...
    config: LogToPartialOrderTransformerConfiguration = {}
  ...
  private addStartAndStopEvent(sequence: PetriNetSequence) {
    ...
    sequence.trace.events.unshift(
      new LogEvent(LogToPartialOrderTransformerService.START_SYMBOL)
    );
    sequence.trace.events.push(
      new LogEvent(LogToPartialOrderTransformerService.STOP_SYMBOL)
    );
  }
  ...
  private performTransitiveReduction(partialOrder: PetriNet) {
    ...
    for (const t of reverseTransitionOrder) {
      ...
      const childrenIds = this.getChildIds(t).sort(
