...
B b`);
    ...
    expect(
      tree.omegaMarking.equals(new Marking({ s: 1, a: 0, b: 0 }))
    ).toBeTrue();

    const children = tree.getChildrenMap();
    ...
    expect(
      child.omegaMarking.equals(new Marking({ s: 0, a: 1, b: 0 }))
    ).toBeTrue();
    ...
    expect(child.ancestors[0]).toBe(tree);
    expect(
      child.omegaMarking.equals(new Marking({ s: 0, a: 0, b: 1 }))
    ).toBeTrue();
    expect(child.getChildren().length).toBe(0);
  });
...
B b2`);
    ...
    expect(tree.ancestors.length).toBe(0);
    expect(
      tree.omegaMarking.equals(new Marking({ a1: 1, b1: 1, a2: 0, b2: 0 }))
    ).toBeTrue();
    ...
    expect(child.ancestors[0]).toBe(tree);
    expect(
      child.omegaMarking.equals(new Marking({ a1: 0, b1: 1, a2: 1, b2: 0 }))
    ).toBeTrue();
    ...
    expect(
      childChild.omegaMarking.equals(
        new Marking({ a1: 0, b1: 0, a2: 1, b2: 1 })
      )
    ).toBeTrue();
    ...
    expect(child.ancestors[0]).toBe(tree);
    expect(
      child.omegaMarking.equals(new Marking({ a1: 1, b1: 0, a2: 0, b2: 1 }))
    ).toBeTrue();
    expect(child.getChildren().length).toBe(1);
...
B a`);
    ...
    const childChild = child.getChildren()[0];
    expect(childChild).toBeTruthy();
    expect(childChild.ancestors.length).toBe(2);
    expect(childChild.ancestors[0]).toBe(tree);
    expect(childChild.ancestors[1]).toBe(child);
    expect(
      childChild.omegaMarking.equals(new Marking({ a: 1, b: 0 }))
    ).toBeTrue();
    expect(childChild.getChildren().length).toBe(0);
  });
...
B a`);
    ...
    expect(tree.ancestors.length).toBe(0);
    expect(
      tree.omegaMarking.equals(new Marking({ a: 1, b: 0, x: 0 }))
    ).toBeTrue();
    expect(tree.getChildren().length).toBe(1);
    ...
    expect(
      child.omegaMarking.equals(new Marking({ a: 0, b: 1, x: 1 }))
    ).toBeTrue();
    expect(child.getChildren().length).toBe(1);
    ...
    expect(
      child.omegaMarking.equals(
        new Marking({ a: 1, b: 0, x: Number.POSITIVE_INFINITY })
      )
    ).toBeTrue();
    expect(child.getChildren().length).toBe(1);
    ancestors.push(child);
    ...
    expect(
      child.omegaMarking.equals(
        new Marking({ a: 0, b: 1, x: Number.POSITIVE_INFINITY })
      )
    ).toBeTrue();
    expect(child.getChildren().length).toBe(1);
    ancestors.push(child);
