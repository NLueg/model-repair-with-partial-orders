...
export class PetriNetCoverabilityService {
  ...
  public getCoverabilityTree(net: PetriNet): CoverabilityTree {
    ...
    whileLoop: while (statesToExplore.length !== 0) {
      ...
      const enabledTransitions = PetriNet.getAllEnabledTransitions(
        net,
        state.omegaMarking
      );
      for (const t of enabledTransitions) {
        const nextMarking = PetriNet.fireTransitionInMarking(
          net,
          t.id!,
          state.omegaMarking
        );
        const nextOmegaMarking = this.computeNextOmegaMarking(
          nextMarking,
          ancestors
        );
  ...
  protected computeNextOmegaMarking(
    nextMarking: Marking,
    ancestors: Array<CoverabilityTree>
  ): Marking {
