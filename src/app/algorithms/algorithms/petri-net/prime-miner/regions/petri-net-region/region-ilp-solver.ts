...
import {
  ...
  MessageLevel,
  Solution,
} from '../../../../../models/glpk/glpk-constants';
...
export class RegionIlpSolver {
  ...
  public computeRegions(
    ...
    const regions$ = new ReplaySubject<Region>(1);
    ...
    ilp$
      ...
      .subscribe((ps: ProblemSolution) => {
        if (ps.solution.result.status === Solution.OPTIMAL) {
          ...
          const nonEmptyInputSet =
            combined.inputs.find((inputs) => inputs.size > 0) ?? [];
  ...
  private setUpInitialILP(
    ...
    initial[config.oneBoundRegions ? 'binaries' : 'generals'] = Array.from(
      this._placeVariables
    );
    this.applyConstraints(
      initial,
      this.createInitialConstraints(combined, config)
    );
  ...
  private createInitialConstraints(
    ...
    result.push(
      ...net
        .getPlaces()
        .map((p) => this.greaterEqualThan(this.variable(p.getId()), 0))
    );

    // non-zero solutions
    result.push(
      this.greaterEqualThan(
        net.getPlaces().map((p) => this.variable(p.getId())),
        1
      )
    );
    ...
    for (const [key, transitions] of labels.entries()) {
      ...
      for (const t2 of transitions) {
        ...
        variables.push(
          ...this.createVariablesFromPlaceIds(
            t2.outgoingArcs.map((a: Arc) => a.destinationId),
            -1
          )
        );
        // t2 pre-set
        variables.push(
          ...this.createVariablesFromPlaceIds(
            t2.ingoingArcs.map((a: Arc) => a.sourceId),
            1
          )
        );
  ...
  private yWhenAGreaterEqualB(
    ...
    return ConstraintsWithNewVariables.combineAndIntroduceVariables(
      ...
      undefined,
      this.greaterEqualThan(
        [this.variable(a), this.variable(y, RegionIlpSolver.K)],
        b
      ),
      this.lessEqualThan(
        [this.variable(a), this.variable(y, RegionIlpSolver.K)],
        RegionIlpSolver.K - 1 + b
      )
    );
  }
