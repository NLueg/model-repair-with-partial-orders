...
import {
  createUniqueString,
  IncrementingCounter,
} from './model/incrementing-counter';
...
export class Relabeler {
  ...
  public uniquelyRelabelSequence(sequence: PartialOrder) {
    this.relabel(sequence, false);
  }

  public uniquelyRelabelSequences(sequences: Iterable<PartialOrder>) {
    iterate(sequences, (s) => {
      this.uniquelyRelabelSequence(s);
    });
  }

  public relabelSequencePreserveNonUniqueIdentities(sequence: PartialOrder) {
    this.relabel(sequence, true);
  }

  public relabelSequencesPreserveNonUniqueIdentities(
    sequences: Iterable<PartialOrder>
  ) {
    iterate(sequences, (s: PartialOrder) => {
      this.relabelSequencePreserveNonUniqueIdentities(s);
    });
  }

  protected relabel(sequence: PartialOrder, preserveIdentities: boolean) {
    this.restartSequence();
    for (let i = 0; i < sequence.events.length; i++) {
      sequence.events[i].label = this.getNewLabel(
        sequence.events[i].label,
        preserveIdentities
      );
    }
  }

  public undoSequenceLabeling(sequence: EditableStringSequenceWrapper) {
    for (let i = 0; i < sequence.length(); i++) {
      sequence.set(i, this.undoLabel(sequence.get(i)));
    }
  }

  public undoSequencesLabeling(
    sequences: Iterable<EditableStringSequenceWrapper>
  ) {
    iterate(sequences, (s) => {
