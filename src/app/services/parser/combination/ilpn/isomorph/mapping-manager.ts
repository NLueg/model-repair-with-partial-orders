...
export class MappingManager {
  ...
  constructor(possibleMappings: MapSet<string, string>) {
    ...
    this._mappingOrder = new Map<string, Array<string>>(
      this._mappingCounters.map((choice) => [
        choice.mappedId,
        Array.from(possibleMappings.get(choice.mappedId)),
      ])
    );
  }
  ...
  public getCurrentMapping(): Map<string, string> {
    return new Map<string, string>(
      this._mappingCounters.map((choice) => [
        choice.mappedId,
        this._mappingOrder.get(choice.mappedId)![choice.current()],
      ])
    );
  }
