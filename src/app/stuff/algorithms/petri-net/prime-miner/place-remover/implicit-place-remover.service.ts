...
import { PetriNetCoverabilityService } from './petri-net-coverability/petri-net-coverability.service';
...
export class ImplicitPlaceRemoverService extends LogCleaner {
  ...
  protected getLabelMapping(net: PetriNet): Map<string, string> {
    ...
    for (const t of net.getTransitions()) {
      if (t.label === undefined) {
        throw new Error(
          `Silent transitions are unsupported! The transition with id '${t.id}' has no label`
        );
      }
      if (result.has(t.label!)) {
        throw new Error(
          `Label splitting is not supported! The label '${t.label}' is shared by at least two transitions`
        );
      }
  ...
  protected stringifyMarking(
    marking: Marking,
    placeOrdering: Array<string>
  ): string {
