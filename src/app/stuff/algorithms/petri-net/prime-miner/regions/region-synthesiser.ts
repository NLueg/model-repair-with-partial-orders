...
export class RegionSynthesiser {
  ...
  public synthesise(): PetriNet {
    if (this._regions.length === 0) {
      throw new Error(
        `You must provide regions via the 'addRegion' method before you can run the synthesis!`
      );
    }
    ...
    for (const t of region.getTransitions()) {
      ...
      if (label === undefined) {
        throw new Error(
          'All transitions in Petri net regions must be labeled!'
        );
      }
    ...
    for (const region of this._regions) {
      const place = new Place(
        region.inputs.reduce(
          (sum, id) => sum + region.net.getPlace(id)!.marking,
          0
        )
      );
      ...
      for (const t of region.net.getTransitions()) {
        ...
        if (existingGradient !== undefined && gradient !== existingGradient) {
          ...
          throw new Error(
            `The provided Petri net is not a valid region! The gradient of label '${label}' of transition with id '${t.id}' is ${gradient}, but a different transition with the same label has a gradient of ${existingGradient}!`
          );
        } else {
  ...
  private isEquivalentPlaceInNet(
    ...
    return net.getPlaces().some((existingPlace) => {
      for (const [label, gradient] of gradients) {
        ...
        if (gradient < 0) {
          if (
            (existingPlace as Place).outgoingArcWeights.get(label) !== -gradient
          ) {
            return false;
          }
        } else if (
          (existingPlace as Place).ingoingArcWeights.get(label) !== gradient
        ) {
          return false;
        }
      }
