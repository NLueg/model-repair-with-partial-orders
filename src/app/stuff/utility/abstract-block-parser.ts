...
export abstract class AbstractBlockParser<T> extends AbstractParser<T> {
  ...
  protected override processFileLines(lines: Array<string>): T | undefined {
    ...
    try {
      for (let i = 0; i < lines.length; i++) {
        ...
        const newBlock = this._supportedBlocks.find((block) =>
  ...
  protected abstract resolveBlockParser(
    block: string
  ): undefined | ((lines: Array<string>, result: T) => void);

  protected parseEachLine(
    lines: Array<string>,
    partParser: (parts: Array<string>, line: string) => void
  ) {
    for (let i = 0; i < lines.length; i++) {
  ...
  private parseBlock(
    ...
    if (currentBlock !== undefined) {
      ...
      if (blockParser === undefined) {
        throw new Error(
          `block type '${currentBlock}' is suppoerted but no block parser could be resolved!`
        );
      }
