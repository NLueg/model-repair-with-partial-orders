    ...
    b.getArcs().forEach((arc) => {
      ...
      if (arc.source instanceof Place) {
        result.addArc(
          new Arc(
            arcId,
            result.getPlace(placeMap.get(arc.sourceId) as string) as Place,
        ...
        result.addArc(
          new Arc(
            ...
            result.getPlace(placeMap.get(arc.destinationId) as string) as Place,
            arc.weight
          )
        );
      }
  ...
  public bindEvents(
    ...
    this._places.forEach((v, k) =>
      v.bindEvents(
        mouseMoved$,
        mouseUp$,
        this._kill$.asObservable(),
        this._redraw$
      )
    );
    this._transitions.forEach((v, k) =>
      v.bindEvents(
        mouseMoved$,
        mouseUp$,
        this._kill$.asObservable(),
