...
import { Arc } from '../pn/model/arc';
import { PetriNet } from '../pn/model/petri-net';
import { Place } from '../pn/model/place';
...
export class PetriNetSerialisationService {
  ...
  private serialiseTransitions(transitions: Array<Transition>): string {
    ...
    transitions.forEach((t) => {
      result += `${this.removeSpaces(t.getId(), t.getId())} ${this.removeSpaces(
        t.label ?? '',
        t.getId()
      )}\n`;
    });
  ...
  private serialiseArcs(arcs: Array<Arc>): string {
    ...
    arcs.forEach((a) => {
      result += `${this.removeSpaces(
        a.sourceId,
        a.getId()
      )} ${this.removeSpaces(a.destinationId, a.getId())}`;
      if (a.weight > 1) {
        result += ` ${a.weight}`;
      }
      result += '\n';
